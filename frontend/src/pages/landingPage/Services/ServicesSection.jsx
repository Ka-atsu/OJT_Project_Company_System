...
import { VIEWPORT_CARDS, EASE, FADE_UP } from "../../../motion/constants";
...
export function ServicesSection({
  ...
  return (
    ...
    >
      ...
      {isPrimary ? (
        <div className="services-hx">
          <div className="services-hx-track">
            {items.map((s, i) => (
              <div className="services-hx-item" key={`${s.title}-${i}`}>
                <motion.div
                  variants={FADE_UP}
                  initial="hidden"
                  whileInView="visible"
        ...
        <Row className={`services-grid services-grid--${variant}`}>
          {items.map((s, i) => (
            <Col md={12} key={`${s.title}-${i}`} className="services-col">
              <motion.div
                variants={FADE_UP}
                initial="hidden"
                whileInView="visible"
